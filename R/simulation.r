...
bsim_gen = function(n, p, seed = NULL, type = c("bayes", "linearA", "linearB", "linearC", "neuralnet", "neuralnet1"))
{
    call = match.call()
    type = match.arg(type)
...
if (type == "neuralnet") {
    ...
    beta_mat1 = matrix(c(2.2, 4.3, -3.5, 2.1,
                        2.5, 1.2, 3.5, -2.0,
                        -4.0, 1.2, 3.0, 2.0),
                        nrow = 3, byrow = TRUE)
    ...
    beta_mat2 = matrix(c(3.1, -1.5, 3.5,
                        2.2, -3.0, 3.2, 
                        1.7, 3.0, 2.1, 
                        2.7, 4.3, -3.1),
                    nrow = node[1], ncol = node[2], byrow = TRUE)
    ...
    output_beta1 = c(1.7, -1.9, 2.5)
    output_beta2 = c(-1.5, 3.1, 1.3)
    
    prob1 = sigmoid(drop(layer2 %*% output_beta1))
    prob2 = sigmoid(drop(layer2 %*% output_beta2))
    probs = cbind(prob1, prob2)
