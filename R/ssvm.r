...
ssvm = function(x = NULL, y = NULL, valid_x = NULL, valid_y = NULL, nfolds = 5,
                  lambda_seq = 2^seq(-10, 10, length.out = 100), 
                  ...
                  scale = FALSE, criterion = c("0-1", "loss"), optModel = FALSE, nCores = 1, verbose = 1, epsilon_D = 1e-4, ...)
{
  out = list()
  cat("Fit c-step \n")
  cstep_fit = cstep.ssvm(x = x, y = y, valid_x = valid_x, valid_y = valid_y, nfolds = nfolds, 
  ...
  cat("Fit theta-step \n")
  thetastep_fit = thetastep.ssvm(cstep_fit, lambda_theta_seq = lambda_theta_seq, isCombined = isCombined, cv_type = "original",  nCores = nCores, epsilon_D = epsilon_D,...)
  
  cat("Fit c-step \n")
  opt_cstep_fit = cstep.ssvm(x = x, y = y, valid_x = valid_x, valid_y = valid_y, nfolds = nfolds, 
...
predict.ssvm = function(object, newx = NULL)
{   
